...
async def draw_abyss_pic(uid: str, nickname: str, floor_num: int, image: Optional[Match] = None, mode: int = 2,
    ...
    text_draw.text((225, 163), f'{nickname}', text_color, genshin_font(30))
    text_draw.text((235, 203), 'UID ' + f'{uid}', text_color, genshin_font(20))
...
async def draw_event_pic() -> None:
    ...
    base_img.save(os.path.join(FILE2_PATH, 'event.jpg'), format='JPEG', subsampling=0, quality=100)
    return
...
async def draw_collect_card(uid: str, nickname: str, image: Optional[str] = None, mode: int = 2) -> str:
    ...
    # 记录数据
    raw_data = raw_data['data']
    max_data = {'成就':892,'华丽的宝箱':185,'珍贵的宝箱':488,'精致的宝箱':1594,'普通的宝箱':2542}
    ...
    # 用户信息
    text_draw.text((130, 60), f'{nickname}', text_color, genshin_font(25), anchor='lm')
    text_draw.text((130, 90), 'UID ' + f'{uid}', text_color, genshin_font(15), anchor='lm')

    text_draw.text((446, 48), str(val), text_color, genshin_font(30), anchor='lm')
